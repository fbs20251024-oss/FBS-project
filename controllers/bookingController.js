...
export const addBooking = async (req, res) => {
  ...
  try {
    ...
    let booking = new Booking({ facility: facilityId, user: userId, date, session });
    await booking.save();
    booking = await Booking.findById(booking._id)
      .populate("user", "username").populate("facility", "facilityName").lean();
    res.status(201).json({
...
export const getBooking = async (req, res) => {
  ...
  try {
    const booking = isUserRoute || isFacilityRoute
      ? await Booking.find(query).sort({ date: 1 }) : await Booking.findOne(query);
    ...
    const response = isUserRoute || isFacilityRoute
      ? { bookings: populatedBooking, total: populatedBooking.length } : { booking: populatedBooking };
...
export const updateBooking = async (req, res) => {
  ...
  try {
    const booking = await Booking.findById(req.params.id)
      .populate("user", "username").populate("facility", "facilityName");
    if (!booking) {
